...
const SCRIPT = [
  ...
  { from: 'me', text: 'Awesome! Here\'s your live site 👇', delay: 1500, link: true },
];

function TypingDots() {
  return <div className="chat__bubble chat__bubble--them chat__typing"><span /><span /><span /></div>;
}
...
export default function ChatMockup() {
  ...
  useEffect(() => {
    ...
    setTyping(true);
    const t1 = setTimeout(() => { setTyping(false); setShown((s) => s + 1); }, next.delay);
    ...
    <div className="chat" ref={ref}>
      <div className="chat__phone">
        <div className="chat__header">
          ...
          <div className="chat__meta"><strong>Saif Ali</strong><span>online</span></div>
        </div>
        <div className="chat__body">
          <AnimatePresence>
            {SCRIPT.slice(0, shown).map((m, i) => (
              <motion.div key={i} className={`chat__bubble chat__bubble--${m.from}`}
                initial={{ opacity: 0, y: 12, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
                {m.link ? (<>Awesome! Here's your live site 👇<span className="chat__link">www.your-restaurant.com</span></>) : m.text}
                <span className="chat__time">{m.from === 'me' && <FiCheck className="chat__tick" />}now</span>
              </motion.div>
            ))}
          </AnimatePresence>
